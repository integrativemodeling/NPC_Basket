...
AMINOACID = [
    'LEU', 'PHE', 'TRP', 'TYR', 'ASN', 'ASP', 'HIS', 'VAL', 'ILE', 'PRO', 'THR',
    'GLY', 'ALA', 'SER', 'CYS', 'GLN', 'GLU', 'MET', 'LYS', 'ARG', 'SOL'
]
...
class PDB:
    ...
    def __init__(self, filename, residue_pairs_dict, chains=[]):
        ...
        self.residue_pairs_dict = residue_pairs_dict
        all_residues = list(residue_pairs_dict.keys())
        ...
        for line in pdblines:
            ...
            #Filtering criteria for specific residues and chain ids
            if line[21:22].strip() not in chains:
                continue

            try:
                resindex = all_residues.index(int(line[22:26]))
            except:
                #print("error")
                continue

            #Append current iter_length
            self.residue_pairs_dict[all_residues[resindex]].append(len(self.__x)-1)
    ...
    def write(self, outfilename):
        ...
        for i in range(self.iter_length()):
            ...
            z = str("%.3f" % self.__z[i])
            fout.writelines(
                pdb_line(serial, name, resName, chainID, resSeq, x, y, z))
        fout.close()
        return
